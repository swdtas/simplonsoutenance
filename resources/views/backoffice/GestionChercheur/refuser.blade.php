...
<div class="row mb-2 text-end mb-xl-3">
    ...
    <h3 class="color1">Listes des chercheurs Réfuser</h3>
</div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            data-target="#chercheurDetailModal{{ $chercheur->id }}">Détail
                            <i class="align-middle" data-feather="eye"></i>
                        </button>

                        <a class="btn btn-danger"
                            href="{{ route('chercheurs.destroy', ['chercheur' => $chercheur->id]) }}"
                            onclick="event.preventDefault(); if(confirm('Voulez-vous vraiment supprimer le chercheur ?')) document.getElementById('delete-form-{{ $chercheur->id }}').submit();">
                            <i class="align-middle" data-feather="trash"></i>
                            Supprimer
                        </a>
                        <form id="delete-form-{{ $chercheur->id }}"
                            action="{{ route('chercheurs.destroy', ['chercheur' => $chercheur->id]) }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                        </form>
